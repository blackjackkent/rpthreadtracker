namespace TumblrThreadTracker.Controllers
{
	...
	public class ThreadController : ApiController
	{
		...
		public void DeleteThreads(List<ThreadDto> threads)
		{
			...
			{
				bool userOwnsThread = _threadService.UserOwnsThread(
					user.UserId,
					thread.UserThreadId.GetValueOrDefault(),
					_threadRepository);
				if (!userOwnsThread) continue;
		...
		public IEnumerable<int?> Get([FromUri] bool isArchived = false)
		{
			...
			foreach (var blog in blogs) ids.AddRange(_threadService.GetThreadIdsByBlogId(blog.UserBlogId, _threadRepository, isArchived));
			return ids;
		}
		...
		{
			var userId = _webSecurityService.GetCurrentUserIdFromIdentity((ClaimsIdentity)User.Identity);
			if (thread == null || userId == null) throw new ArgumentNullException();
		...
		{
			var user = _webSecurityService.GetCurrentUserFromIdentity((ClaimsIdentity)User.Identity);
			if (thread == null || thread.UserThreadId == null || user == null) return new HttpResponseMessage(HttpStatusCode.BadRequest);
			bool userOwnsThread = _threadService.UserOwnsThread(
				user.UserId,
				thread.UserThreadId.GetValueOrDefault(),
				_threadRepository);
