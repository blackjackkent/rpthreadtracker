...
rpThreadTracker.controllers.controller('EditBlogController', [
    '$scope', '$routeParams', '$location', 'sessionService', 'contextService', 'blogService', 'threadService', 'TrackerNotification', 'pageId',
    function ($scope, $routeParams, $location, sessionService, contextService, blogService, threadService, TrackerNotification, pageId) {
        $scope.setBodyClass('');
        $scope.pageId = pageId;
        ...
        function initView(userBlogId) {
            blogService.getBlogs(true, true).then(function (blogs) {
                ...
                $scope.blogToEdit = angular.copy(_.find(blogs, function(blog) { return blog.UserBlogId == userBlogId }));
