...
rpThreadTracker.controllers.controller('ManageBlogsController', [
    '$scope', '$location', 'sessionService', 'blogService', 'threadService', 'pageId', 'TrackerNotification', function($scope, $location, sessionService, blogService, threadService, pageId, TrackerNotification) {
        $scope.setBodyClass('');
        $scope.pageId = pageId;
        ...
        function createBlog() {
            ...
            var shortnameExists = _.findIndex($scope.blogs, function (blog) { return blog.BlogShortname == $scope.newBlogShortname; }) !== -1;
            if (shortnameExists) {
                $scope.duplicateErrorNotification = new TrackerNotification()
                    .withMessage("ERROR: A blog with the shortname '<em>" + $scope.newBlogShortname + "</em>' is already associated with your account.")
                    .withType("error")
                    .show();
                return;
            }
